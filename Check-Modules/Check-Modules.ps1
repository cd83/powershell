...
.SYNOPSIS
This script installs custom modules saved in the same working directory and updates them as needed.

.DESCRIPTION
This script checks each individual folder in the repository for a module and installs or updates it on the local machine.
...
Function verCompare($ver1, $ver2) {
 ...
 foreach ($File in $psdFiles) {
     ...
     Import-Module $("$env:USERPROFILE\Documents\WindowsPowerShell\Modules\$((Get-Item $File).name.split('.')[0])")
     ""
 }
